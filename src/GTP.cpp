...
bool GTP::execute(GameState & game, std::string xinput) {
    ...
    if (command == "protocol_version") {
        ...
        if (command.find("alive") != std::string::npos) {
            std::string livelist = "Not implemented!\n";
            gtp_printf(id, livelist.c_str());
        } else if (command.find("dead") != std::string::npos) {
            std::string deadlist = "Not implemented\n";
        ...
        if (!cmdstream.fail()) {
            ...
            game.set_timecontrol(maintime * 100);
        ...
        if (!cmdstream.fail()) {
            ...
            game.adjust_time(icolor, time * 100);
        ...
        do {
            ...
            game.play_move(move);
            game.display_state();
            // the condition of game over : both pass or resign;
            //note that in leelaz, there is always a pass action,
            //so usr can use pass to contain the possibility of full board.
            //but in othello it's different, we can't choose to pass except thai there's no other action
        ...
        if (tc_type.find("none") != std::string::npos) {
            // 30 mins
            game.set_timecontrol(30 * 60 * 100);

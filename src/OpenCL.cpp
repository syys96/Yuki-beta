...
static std::string sourceCode_convolve3 = R"(
    ...
    void convolve3(
        ...
        const int filter_len = filter_size * filter_size;//9
        const int mid = (filter_size / 2) + 1;//2
        const int extent = mid - 1;//1
        const int pad_width = width + filter_size - 1;//21->10
        ...
        for (int tile = 0; tile < row_tile_size; tile++) {
            ...
            if (row > BOARD_SIZE - 1) break;

            // Copy the input channels (strips) locally
            if (out_buff_size < 10 && ly == 0) {
                // strip-row
            ...
            } else if (out_buff_size >= 10 && ly < 10) {
                // Every thread copies a column
                int copy_idx = (lx * pad_width + ly) * filter_size;
                if (tile == 0 || row == BOARD_SIZE - 1) {
                    // Every thread copies a column
                    for (int srow = 0; srow < filter_size; srow++) {
                        int in_row = row - extent + srow;
                        float val = 0.0f;

...
static std::array<float, SQUARE_NUM * 2 * ACTION_N> ip_pol_w;
static std::array<float, ACTION_N> ip_pol_b;
...
static std::array<float, 256 * SQUARE_NUM> ip1_val_w;
...
void Network::initialize(void) {
    ...
    opencl_net.push_batchnorm(SQUARE_NUM, batchnorm_means[weight_index],
...
Network::Netresult Network::get_scored_moves_internal(
    ...
    std::vector<float> policy_data_1(2 * SQUARE_NUM);
    std::vector<float> policy_data_2(2 * SQUARE_NUM);
    std::vector<float> value_data_1(1 * SQUARE_NUM);
    std::vector<float> value_data_2(1 * SQUARE_NUM);
    std::vector<float> policy_out(ACTION_N);
    std::vector<float> softmax_data(ACTION_N);
    ...
    batchnorm<2, SQUARE_NUM>(policy_data_1, bn_pol_w1, bn_pol_w2, policy_data_2);
    innerproduct<2*SQUARE_NUM, ACTION_N>(policy_data_2, ip_pol_w, ip_pol_b, policy_out);
    softmax(policy_out, softmax_data, cfg_softmax_temp);
    std::vector<float>& outputs = softmax_data;

    // Now get the score
    convolve<1, 1>(output_data, conv_val_w, conv_val_b, value_data_1);
    batchnorm<1, SQUARE_NUM>(value_data_1, bn_val_w1, bn_val_w2, value_data_2);
    innerproduct<SQUARE_NUM, 256>(value_data_2, ip1_val_w, ip1_val_b, winrate_data);
...
void Network::gather_features(GameState * state, NNPlanes & planes) {
    planes.resize(INPUT_CHANNEL);
    constexpr size_t our_offset   = 0;
    constexpr size_t their_offset = PLANE_SIDE;
    BoardPlane& black_to_move  = planes[PLANE_NUM];
    BoardPlane& white_to_move  = planes[PLANE_NUM + 1];

...
void Training::record(GameState& state, UCTNode& root) {
    ...
    step.probabilities.resize(ACTION_N);
    ...
    while (child != nullptr) {
        ...
        // pass is not included in the probabilities.
        child = child->get_sibling();
    }
...
void Training::dump_training(int winner_color, OutputChunker& outchunk) {
    for (const auto& step : m_data) {
        ...
        for (auto p = size_t{0}; p < PLANE_NUM; p++) {
            ...
            // 361 % 4 = 1 so the last bit goes by itself in leelaz,
            // 64 % 4 = 0 so there's no last bit in yuki.
        ...
        // Then a 64 long array of float probabilities
...
void Training::process_game(GameState& state, size_t& train_pos, int who_won,
    ...
    do {
        ...
        if (skip == 0) {
            ...
            step.probabilities.resize(ACTION_N);
            if (this_move != BOARD_SIZE * BOARD_SIZE)
                step.probabilities[this_move] = 1.0f;
            //If this move is pass, the probalities should be all zeros,
            //which can be regraded as the requirements of rules.
            train_pos++;
            m_data.emplace_back(step);
        }
